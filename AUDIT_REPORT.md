...
| **HIGH (correctness)** | `backend/app/repositories/profitability.py:15` + `closing.py:14` | **Month bounds built in UTC** while `executed_at` is a UTC instant whose *local* month is Europe/Athens → executions in the ~2–3h around a month edge mis-counted into the adjacent month (financial totals slightly off). | Build bounds in `ZoneInfo("Europe/Athens")` (comparison stays instant-based). **Confirmed by Panagiotis** — slightly shifts boundary monthly totals (corrects them). |
| **MED (security)** | `backend/app/api/v1/routers/onboarding.py:55` | Public, unauthenticated ΑΑΔΕ VAT lookup (`/aade/{afm}`) had **no rate limit** → AFM enumeration / ΑΑΔΕ quota exhaustion. | Added `rate_limit("aade_lookup", limit=10, window_seconds=3600)` (10/hour/IP, fail-open). No change for normal signup. |
| **MED (security)** | `backend/app/core/config.py:74` (`assert_production_secrets`) | Prod boot gate rejected dev-default/empty secrets but not **weak short** ones. | Now also rejects `JWT_SECRET`/`JWT_PLATFORM_SECRET`/pepper **< 32 chars** in prod. ⚠️ Fails prod boot on a weak secret (intended) — verify prod secrets are ≥32 before deploy. Dev unaffected. |
...
| **MED** | `infra_cloud.py` `/verify` (live cloud token checks, admin) | Repeated calls could trigger provider lockout/rate-limits. | Add `rate_limit(...)`. (Public ΑΑΔΕ lookup now rate-limited — see FIXED.) |
