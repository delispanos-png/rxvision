"""Data retention — ΗΜΕΡΟΛΟΓΙΑΚΑ ΕΤΗ ΑΝΑ ΦΑΡΜΑΚΕΙΟ.
...
BASE_RETENTION_YEARS = 3         # τρέχον + 2 ΠΛΗΡΗ προηγούμενα — το βασικό (χωρίς χρέωση)
DEFAULT_RETENTION_MONTHS = 36    # η ίδια συμφωνία σε μήνες — μένει ως πεδίο ΧΡΕΩΣΗΣ
...
def cutoff_for(months: int, now: datetime | None = None) -> datetime:
    ...
    ΗΜΕΡΟΛΟΓΙΑΚΑ ΕΤΗ, ΟΧΙ ΚΥΛΙΟΜΕΝΟΙ ΜΗΝΕΣ (απόφαση ιδιοκτήτη 19/09/2026): κάθε φαρμακείο
    κρατά την ΤΡΕΧΟΥΣΑ χρονιά + τις 2 ΠΛΗΡΕΙΣ προηγούμενες. Μέσα στο 2026 → από 01/01/2024.
    Την 01/01/2027 το 2024 φεύγει ΟΛΟΚΛΗΡΟ και μένουν 2025, 2026 + η τρέχουσα.

    ΓΙΑΤΙ ΑΛΛΑΞΕ: το κυλιόμενο παράθυρο έδινε κινούμενο στόχο (π.χ. 19/09/2023) — ο πελάτης
    έβλεπε «μισό 2023» που λιγόστευε κάθε μέρα, και η σύγκριση με πέρυσι ήταν ασύμμετρη.
    Με ημερολογιακά έτη, ό,τι βλέπει είναι πάντα ΟΛΟΚΛΗΡΕΣ χρονιές.

    Το `months` ΔΕΝ καταργείται: παραμένει το πεδίο ΧΡΕΩΣΗΣ, απ' όπου βγαίνουν τα αγορασμένα
    επιπλέον έτη (36μ→0, 48μ→+1, 60μ→+2). Όποιος έχει αγοράσει, κρατά αναλόγως περισσότερα.
    """
    now = now or _utcnow()
    years = BASE_RETENTION_YEARS + extra_years(months)
    return datetime(now.year - (years - 1), 1, 1, tzinfo=timezone.utc)
