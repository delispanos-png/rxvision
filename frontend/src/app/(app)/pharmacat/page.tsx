...
type Product = { name: string; barcode: string; atc: string; retail: number; wholesale: number; margin: number; narcotic: boolean; category: string };
      ...
      <div className="border-t border-slate-200 pt-3 dark:border-slate-700">
        <div className="flex gap-2">
          <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(input); } }}
            disabled={busy || notConfigured} placeholder={t("Σύμπτωμα ή κλινική ερώτηση…", "Symptom or clinical question…")}
          ...
          <button onClick={() => send(input, "interactions")} disabled={busy || notConfigured || !input.trim()} title={t("Έλεγχος αλληλεπιδράσεων (φάρμακα χωρισμένα με κόμμα)", "Interaction check (comma-separated drugs)")} className="grid place-items-center rounded-xl border border-slate-300 px-3 text-slate-500 hover:bg-slate-50 disabled:opacity-40 dark:border-slate-600"><FlaskConical className="h-4 w-4" /></button>
          <button onClick={() => send(input)} disabled={busy || notConfigured || !input.trim()} className="grid place-items-center rounded-xl bg-violet-600 px-4 text-white hover:bg-violet-700 disabled:opacity-40"><Send className="h-4 w-4" /></button>
        </div>
...
function AssistantCard({ r, t, onAnswer }: { r: Result; t: (el: string, en: string) => string; onAnswer: (a: string) => void }) {
  ...
  return (
    <>
      ...
      {!!r.products?.length && (
        <div className="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-900">
          <div className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300"><Package className="h-3.5 w-3.5 text-violet-500" /> {t("Διαθέσιμα προϊόντα (κατάλογος)", "Available products (catalogue)")}</div>
          <div className="space-y-2">{r.products.map((g, i) => (
            <div key={i}>
              <div className="text-[11px] font-medium text-slate-400">{g.substance}</div>
              {g.products.map((p, j) => (
                <div key={j} className="flex items-center justify-between gap-2 border-t border-slate-100 py-1 text-xs first:border-0 dark:border-slate-800">
                  <span className="truncate text-slate-700 dark:text-slate-200">{p.name} {p.narcotic && <span className="text-rose-500">⚠ ναρκωτικό</span>}</span>
                  <span className="shrink-0 text-right"><b>{fmtEur(p.retail)}</b> <span className="text-emerald-600">+{fmtEur(p.margin)}</span></span>
                </div>
              ))}
            </div>
          ))}</div>
