...
const MODULE_LABELS: [string, string][] = [
  ...
  ["daily_coach", "🧭 Ο Σύμβουλός σου (καθημερινή αξιολόγηση)"],
...
export default function TenantCardPage() {
  ...
  async function runDeleteAll() {
    ...
    setCopyBusy(true);
    try {
      const r = await adminApi<{ ok: boolean; deleted: number }>(`/admin/tenants/${encodeURIComponent(id)}/items`, { method: "DELETE" });
      await appAlert(`✓ Διαγράφηκαν ${r.deleted} είδη.`, { title: "Διαγραφή ειδών" });
    } catch { await appAlert("Αποτυχία διαγραφής."); }
    setCopyBusy(false);
  }
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-6 rounded-xl border border-slate-200 bg-white p-6">
        ...
        {/* Εμπλουτισμός ειδών από προμηθευτές — ΟΧΙ «από βάση ειδών»: σήμερα η πηγή είναι ο βάση ειδών,
        ...
        <div className="mt-3 rounded-lg border border-indigo-200 bg-indigo-50/50 p-3">
          <label className="flex cursor-pointer items-center justify-between text-sm">
            <span className="text-slate-700"><b>📷 Εμπλουτισμός ειδών από προμηθευτές</b> <span className="text-xs text-slate-400">(back-office· φωτο/περιγραφή/ΦΠΑ/τιμές από τη βάση ειδών — μόνο στη βάση αυτού του φαρμακείου)</span></span>
            <input type="checkbox" checked={!!profarmQ.data?.enabled} disabled={busy}
              onChange={(e) => { const v = e.target.checked; act(() => adminApi(`/admin/tenants/${encodeURIComponent(id)}/profarm-module?enabled=${v}`, { method: "POST" }), v ? "Εμπλουτισμός ειδών ΟΝ ✓" : "Εμπλουτισμός ειδών OFF ✓").then(() => profarmQ.refetch()); }}
              className="h-4 w-4 accent-indigo-600" />
          </label>
        </div>
      </div>
