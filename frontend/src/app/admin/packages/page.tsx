              ...
              <div className="space-y-4 border-t border-slate-100 bg-slate-50/40 px-4 py-4">
                ...
                <section className="rounded-xl border border-slate-200 bg-white p-4">
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <label className="block text-xs font-medium text-slate-500">🤖 Δωρεάν AI ερωτήσεις<input type="number" min={0} className={`mt-1 ${inp}`} disabled={p.ai_free_enabled === false} value={p.ai_free_enabled === false ? 0 : (p.ai_included ?? 0)} onChange={(e) => setP(p._id, { ai_included: parseInt(e.target.value) || 0 })} /></label>
