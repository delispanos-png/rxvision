...
type S1 = {
  ...
  series?: string; form?: string; issuer_afm?: string; issuer_name?: string; configured?: boolean;
};
...
export default function AdminSoftonePage() {
  ...
  const [f, setF] = useState({ base_url: "", app_id: "", username: "", password: "", company: "", branch: "", module: "", refid: "", series: "", form: "", issuer_afm: "", issuer_name: "" });
  const [notice, setNotice] = useState<string | null>(null);
  const [test, setTest] = useState<string | null>(null);

  useEffect(() => {
    if (!s1) return;
    setF((p) => ({ ...p, base_url: s1.base_url || "", app_id: s1.app_id || "", username: s1.username || "", company: s1.company || "", branch: s1.branch || "", module: s1.module || "", refid: s1.refid || "", series: s1.series || "", form: s1.form || "", issuer_afm: s1.issuer_afm || "", issuer_name: s1.issuer_name || "" }));
  }, [s1]);
  ...
  const save = useMutation({
    mutationFn: () => adminApi("/admin/integrations", { method: "PUT", body: JSON.stringify({
      ...
      softone_series: f.series, softone_form: f.form, softone_issuer_afm: f.issuer_afm, softone_issuer_name: f.issuer_name,
    }) }),
