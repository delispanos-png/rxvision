...
function errText(code?: string): string {
  switch (code) {
    case "email_exists": return "Υπάρχει ήδη λογαριασμός με αυτό το email.";
    case "amka_exists": return "Υπάρχει ήδη λογαριασμός με αυτό το ΑΜΚΑ.";
    case "approval_required":
      return "Δεν βρέθηκαν στοιχεία επικοινωνίας σου σε κάποιο φαρμακείο του δικτύου, ώστε να επιβεβαιώσουμε την ταυτότητά σου. Ζήτησε από το φαρμακείο σου να δημιουργήσει τον λογαριασμό σου.";
    case "otp_send_failed": return "Δεν στάλθηκε ο κωδικός. Δοκίμασε ξανά σε λίγο.";
    case "otp_invalid": return "Λάθος κωδικός. Δοκίμασε ξανά.";
    case "otp_expired": return "Ο κωδικός έληξε. Ξεκίνα ξανά την εγγραφή.";
    case "otp_locked": return "Πολλές λάθος προσπάθειες. Ξεκίνα ξανά την εγγραφή.";
    default: return "Κάτι πήγε στραβά. Έλεγξε τα στοιχεία.";
  }
}
    ...
    <div className="h-full overflow-y-auto"><div className="flex min-h-full items-center justify-center px-4 py-8">
      {step === "form" ? (
        <form onSubmit={submit} className="w-full max-w-sm space-y-3 rounded-3xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/60 sm:p-8">
          ...
          <p className="rounded-xl bg-brand-50 px-3 py-2 text-[11px] text-brand-600">🔒 Για την ασφάλειά σου, θα στείλουμε έναν κωδικό επιβεβαίωσης στο τηλέφωνο ή email που έχει το φαρμακείο σου καταχωρημένο για εσένα.</p>
          <button type="submit" disabled={busy} className="w-full rounded-xl bg-brand-600 py-3 text-sm font-semibold text-white shadow-sm shadow-brand-500/30 hover:bg-brand-700 disabled:opacity-60">
            {busy ? "Αποστολή κωδικού…" : "Συνέχεια"}
          </button>
        ...
        <form onSubmit={verify} className="w-full max-w-sm space-y-3 rounded-3xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/60 sm:p-8">
          <div className="text-center">
            ...
            <h1 className="text-xl font-extrabold tracking-tight text-slate-900">Επιβεβαίωση ταυτότητας</h1>
            <p className="mt-1 text-sm text-slate-500">
              Στείλαμε 6ψήφιο κωδικό {channels.length ? "στο " + channels.map((c) => c.hint).join(" και ") : ""}. Πληκτρολόγησέ τον για να ολοκληρωθεί η εγγραφή.
            </p>
          </div>
          {err && <div className="rounded-xl bg-rose-50 px-3 py-2 text-sm text-rose-700">{err}</div>}
          ...
          <button type="submit" disabled={busy || code.length < 4} className="w-full rounded-xl bg-brand-600 py-3 text-sm font-semibold text-white shadow-sm shadow-brand-500/30 hover:bg-brand-700 disabled:opacity-60">
            {busy ? "Επιβεβαίωση…" : "Ολοκλήρωση εγγραφής"}
          </button>
