      ...
      <div className="space-y-2.5 md:hidden">
        {pageRows.map((row, i) => {
          ...
          return (
            ...
            >
              <div className="mb-2 break-words text-[15px] font-semibold text-slate-900">{cell(title, row)}</div>
              {gridCols.length > 0 && (
                <div className="grid grid-cols-1 gap-x-4 gap-y-1.5 sm:grid-cols-2">
                  {gridCols.map((c) => (
                    <div key={c.key} className="flex min-w-0 items-center justify-between gap-2 border-b border-slate-50 pb-1 last:border-0">
                      <span className="shrink-0 text-xs text-slate-400">{c.header}</span>
                      <span className="truncate text-right text-sm font-medium text-slate-700">{cell(c, row)}</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="mt-3 flex items-center justify-between text-sm">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={safePage <= 1}
              className="rounded-lg border border-slate-300 px-3 py-2 font-medium text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40 sm:py-1.5"
