...
export function ModuleGuard({ module, children }: { module: string; children: React.ReactNode }) {
  ...
  if (isLoading) return <div className="p-6 text-slate-400">{t("Φόρτωση…", "Loading…")}</div>;
  ...
  if (isError) {
    return (
      <div className="rounded-xl border border-rose-200 bg-rose-50 p-8 text-center">
        <p className="text-rose-700">{t("Δεν ήταν δυνατή η φόρτωση. Ελέγξτε τη σύνδεσή σας.", "Could not load. Check your connection.")}</p>
        <button
          onClick={() => refetch()}
