  ...
  async function pick(p: Pharmacy) {
    ...
    try {
      ...
      if (prob?.detail?.error === "seat_limit") {
        setErr(t(`Δεν μπορείς να συνδεθείς στο «${p.name}» γιατί δεν υπάρχουν διαθέσιμες άδειες — είναι όλες κατειλημμένες${prob.detail.seats ? ` (${prob.detail.seats})` : ""}.`,
                 `You can't switch to “${p.name}” because there are no available seats — all are taken${prob.detail.seats ? ` (${prob.detail.seats})` : ""}.`));
      } else {
        setErr(t("Δεν ήταν δυνατή η αλλαγή φαρμακείου.", "Could not switch pharmacy."));
      }
      setBusy("");
    }
  }
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 z-50 mt-1.5 max-h-[70vh] w-[min(18rem,calc(100vw-1.5rem))] overflow-auto rounded-xl border border-slate-200 bg-white p-1 shadow-xl dark:border-slate-700 dark:bg-slate-800">
            <div className="px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-slate-400">{t("Τα φαρμακεία μου", "My pharmacies")}</div>
            {pharmacies.map((p) => {
              ...
              return (
                <button key={p.tenant_id} onClick={() => pick(p)} disabled={!!busy}
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    {p.primary && <span className="text-[10px] text-slate-400">{t("κύριο", "main")}</span>}
                  </span>
