 ...
 */
/** Το ΠΕΡΙΕΧΟΜΕΝΟ του παραθύρου — ίδιο σε πελάτη και σε προεπισκόπηση adminpanel, ώστε αυτό
 *  που βλέπεις πριν δημοσιεύσεις να είναι ακριβώς αυτό που θα δει εκείνος. */
export function AnnouncementCard({ ann, onAction, onClose, done, note, setNote, busy }: {
  ann: Ann; onAction: (a: "trial" | "demo" | "info" | "never" | "dismissed") => void;
  onClose: () => void; done: "trial" | "demo" | null;
  note: string; setNote: (v: string) => void; busy?: boolean;
}) {
  const t = useT();
  const a = ann.addon;
  ...
  return (
    <div className="w-full max-w-lg overflow-hidden rounded-3xl bg-white shadow-2xl dark:bg-slate-900">
      <div className="relative bg-gradient-to-br from-indigo-600 to-sky-600 px-6 py-5 text-white">
        <button onClick={onClose} aria-label={t("Κλείσιμο", "Close")}
        ...
        <h2 className="mt-1.5 pr-8 text-xl font-bold leading-snug">
          {a?.icon ? `${a.icon} ` : ""}{ann.title || t("(χωρίς τίτλο)", "(untitled)")}
        </h2>
      </div>
      ...
      {done ? (
        ...
        <div className="px-6 py-5">
          {ann.body && <p className="whitespace-pre-line text-sm leading-relaxed text-slate-700 dark:text-slate-300">{ann.body}</p>}
          {a?.description && !ann.body && <p className="text-sm leading-relaxed text-slate-700 dark:text-slate-300">{a.description}</p>}
          {!!a?.features?.length && (
          ...
          {(ann.cta?.trial !== false || ann.cta?.demo !== false) && (
            <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={2}
              placeholder={t("Θέλεις να μας πεις κάτι; (προαιρετικό — π.χ. πότε σε βολεύει)", "Anything to add? (optional)")}
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {ann.cta?.trial !== false && (
              <button onClick={() => onAction("trial")} disabled={busy}
            ...
            {ann.cta?.demo !== false && (
              <button onClick={() => onAction("demo")} disabled={busy}
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            {ann.cta?.info_href ? (
              <Link href={ann.cta.info_href} onClick={() => onAction("info")}
...
export function AnnouncementPopup() {
  ...
  return (
    <div className="fixed inset-0 z-[70] grid place-items-center bg-slate-900/50 p-4 backdrop-blur-sm"
      ...
      <AnnouncementCard ann={ann} done={done} note={note} setNote={setNote} busy={act.isPending}
        onClose={() => setOpen(false)}
        onAction={(x) => {
          if (x === "trial" || x === "demo") ask(x);
          else close(x);
        }} />
    </div>
  );
