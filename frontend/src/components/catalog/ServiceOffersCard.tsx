  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      <div className="mb-1 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 font-semibold text-slate-800"><Sparkles className="h-4 w-4 text-fuchsia-500" /> {t("Προσφορές υπηρεσιών", "Service offers")}</div>
      ...
      <p className="mb-3 text-xs text-slate-500">{t("Προσφορές σε ", "Offers on ")}<b>{t("υπηρεσίες", "services")}</b>{t(" (μέτρηση πίεσης, σπιρομέτρηση, φαρμακευτική συμβουλή…). Ο πελάτης τις βλέπει στις «Προσφορές» της πύλης και ", " (blood-pressure check, spirometry, pharmaceutical advice…). Customers see them in the portal «Offers» and ")}<b>{t("κλείνει ραντεβού", "book an appointment")}</b>{t(" — δεν μπαίνουν στο καλάθι.", " — they don't go into the cart.")}</p>

      {items.length === 0 && <p className="py-4 text-center text-sm text-slate-400">{t("Καμία προσφορά υπηρεσίας ακόμη.", "No service offers yet.")}</p>}
          ...
          <div key={o._id} className={`flex items-start gap-3 rounded-xl border p-3 ${o.active ? "border-fuchsia-200 bg-fuchsia-50/40" : "border-slate-200 bg-slate-50"}`}>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                {!o.active && <span className="rounded-full bg-slate-200 px-2 py-0.5 text-[10px] font-semibold text-slate-600">{t("Ανενεργή", "Off")}</span>}
              </div>
              <div className="mt-0.5 text-sm">
                {o.is_free ? <span className="font-bold text-emerald-600">{t("Δωρεάν", "Free")}</span> : (
                ...
                <span className="ml-2 text-[11px] text-slate-400">{o.cta === "reserve" ? t("· κράτηση ραντεβού", "· book appointment") : t("· ενημερωτική", "· informational")}</span>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setEdit(null)}>
          <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              <div className="font-semibold text-slate-800">{edit._id ? t("Επεξεργασία προσφοράς", "Edit offer") : t("Νέα προσφορά υπηρεσίας", "New service offer")}</div>
            ...
            <div className="space-y-3">
              <label className="block text-xs text-slate-500">{t("Τίτλος", "Title")}
                <input value={edit.title} onChange={(e) => setEdit({ ...edit, title: e.target.value })} placeholder={t("π.χ. Σπιρομέτρηση −30%", "e.g. Spirometry −30%")} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" /></label>
              <label className="block text-xs text-slate-500">{t("Περιγραφή (προαιρετικό)", "Description (optional)")}
              ...
              <div className="flex items-center gap-4">
                ...
                <label className="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-600 hover:bg-slate-50">
                  {uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <ImagePlus className="h-4 w-4" />} {t("Εικόνα", "Image")}
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && pickImage(e.target.files[0])} /></label>
                {imgSrc(edit) && <button onClick={() => setEdit({ ...edit, image_id: null, photo_url: null })} className="text-[11px] text-rose-500 hover:underline">{t("Αφαίρεση", "Remove")}</button>}
              </div>
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={edit.is_free} onChange={(e) => setEdit({ ...edit, is_free: e.target.checked })} /> {t("Δωρεάν υπηρεσία", "Free service")}</label>
              {!edit.is_free && (
                <div className="grid grid-cols-2 gap-3">
                  <label className="text-xs text-slate-500">{t("Τιμή τώρα (€)", "Price now (€)")}
                  ...
                  <label className="text-xs text-slate-500">{t("Τιμή πριν (€, προαιρετικό)", "Price before (€, optional)")}
              ...
              <div className="flex items-center justify-between gap-3">
                <label className="text-xs text-slate-500">{t("Ενέργεια πελάτη", "Customer action")}
                  <select value={edit.cta} onChange={(e) => setEdit({ ...edit, cta: e.target.value as "reserve" | "info" })} className="mt-1 block rounded-lg border border-slate-300 px-3 py-2 text-sm">
                    <option value="reserve">{t("Κράτηση ραντεβού", "Book appointment")}</option>
                    <option value="info">{t("Μόνο προβολή (ρώτα στο φαρμακείο)", "View only (ask at the pharmacy)")}</option>
                  </select></label>
                <label className="mt-4 flex items-center gap-2 text-sm"><input type="checkbox" checked={edit.active} onChange={(e) => setEdit({ ...edit, active: e.target.checked })} /> {t("Ενεργή", "Active")}</label>
              </div>
            </div>
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button onClick={() => edit.title.trim().length >= 2 && save.mutate(edit)} disabled={save.isPending} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-50">{save.isPending ? t("Αποθήκευση…", "Saving…") : t("Αποθήκευση", "Save")}</button>
            </div>
          </div>
        </div>
      )}
