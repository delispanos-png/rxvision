...
export function PromosCard() {
  ...
  const saveC = useMutation({
    ...
    onSuccess: (r) => { if (r?.ok === false) { setErr(r.error === "code_exists" ? t("Ο κωδικός υπάρχει ήδη.", "This code already exists.") : t("Σφάλμα.", "Error.")); return; } qc.invalidateQueries({ queryKey: cKey }); setEc(null); setErr(""); },
  });
  ...
  const saveB = useMutation({
    ...
    onSuccess: (r) => { if (r?.ok === false) { setErr(r.error === "need_two_lines" ? t("Το combo θέλει ≥2 barcodes.", "A combo needs ≥2 barcodes.") : t("Σφάλμα — έλεγξε το barcode.", "Error — check the barcode.")); return; } qc.invalidateQueries({ queryKey: bKey }); setEb(null); setErr(""); },
  });
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        <div className="mb-1 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 font-semibold text-slate-800"><Ticket className="h-4 w-4 text-violet-500" /> {t("Κουπόνια", "Coupons")}</div>
          <button onClick={() => { setEc({ ...EMPTY_C }); setErr(""); }} className="inline-flex items-center gap-1 rounded-lg bg-brand-600 px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-brand-700"><Plus className="h-3.5 w-3.5" /> {t("Νέο", "New")}</button>
        </div>
        <p className="mb-3 text-xs text-slate-500">{t("Κωδικός έκπτωσης στο καλάθι. Ισχύει μόνο στα μη-συνταγογραφούμενα.", "Cart discount code. Applies only to non-prescription items.")}</p>
        {cs.length === 0 && <p className="py-3 text-center text-sm text-slate-400">{t("Κανένα κουπόνι.", "No coupons.")}</p>}
            ...
            <div key={c._id} className={`flex items-center justify-between gap-2 rounded-xl border p-2.5 ${c.active ? "border-violet-200 bg-violet-50/50" : "border-slate-200 bg-slate-50"}`}>
              <div className="min-w-0">
                ...
                <div className="mt-0.5 text-[11px] text-slate-500">
                  {c.min_order_cents > 0 && <>{t("ελάχ.", "min.")} {eur(c.min_order_cents)} · </>}
                  {c.max_uses > 0 ? t(`${c.used_count ?? 0}/${c.max_uses} χρήσεις`, `${c.used_count ?? 0}/${c.max_uses} uses`) : t(`${c.used_count ?? 0} χρήσεις`, `${c.used_count ?? 0} uses`)}
                  {c.expires_at && <> · {t("λήγει", "expires")} {new Date(c.expires_at).toLocaleDateString("el-GR")}</>}
                </div>
              </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        <div className="mb-1 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 font-semibold text-slate-800"><Package className="h-4 w-4 text-emerald-500" /> {t("Πακέτα", "Bundles")}</div>
          <button onClick={() => { setEb({ ...EMPTY_B }); setErr(""); }} className="inline-flex items-center gap-1 rounded-lg bg-brand-600 px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-brand-700"><Plus className="h-3.5 w-3.5" /> {t("Νέο", "New")}</button>
        </div>
        <p className="mb-3 text-xs text-slate-500">{t("«2+1» στο ίδιο είδος ή combo (μαζί φθηνότερα). Μόνο μη-συνταγογραφούμενα.", "«2+1» on the same item or a combo (cheaper together). Non-prescription only.")}</p>
        {bs.length === 0 && <p className="py-3 text-center text-sm text-slate-400">{t("Κανένα πακέτο.", "No bundles.")}</p>}
        <div className="space-y-2">
          {bs.map((b) => (
      ...
      {ec && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={() => setEc(null)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-5" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between"><div className="font-semibold text-slate-800">{ec._id ? t("Επεξεργασία κουπονιού", "Edit coupon") : t("Νέο κουπόνι", "New coupon")}</div><button onClick={() => setEc(null)} className="text-slate-400"><X className="h-4 w-4" /></button></div>
            <div className="space-y-3">
              <label className="block text-sm"><span className="mb-1 block text-slate-600">{t("Κωδικός", "Code")}</span>
              ...
              <div className="flex gap-2">
                {(["pct", "amount"] as const).map((k) => (
                  <button key={k} onClick={() => setEc({ ...ec, kind: k, value: k === "pct" ? 10 : 500 })} className={`flex-1 rounded-lg border px-2 py-2 text-sm font-semibold ${ec.kind === k ? "border-violet-500 bg-violet-600 text-white" : "border-slate-200 bg-white text-slate-600"}`}>{k === "pct" ? t("Ποσοστό %", "Percent %") : t("Ποσό €", "Amount €")}</button>
                ))}
              </div>
              <label className="block text-sm"><span className="mb-1 block text-slate-600">{ec.kind === "pct" ? t("Έκπτωση %", "Discount %") : t("Έκπτωση (€)", "Discount (€)")}</span>
              ...
              <label className="block text-sm"><span className="mb-1 block text-slate-600">{t("Ελάχιστη αξία καλαθιού (€) — 0 = καμία", "Minimum cart value (€) — 0 = none")}</span>
              ...
              <label className="block text-sm"><span className="mb-1 block text-slate-600">{t("Μέγιστες χρήσεις — 0 = απεριόριστες", "Maximum uses — 0 = unlimited")}</span>
              ...
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={ec.active} onChange={(e) => setEc({ ...ec, active: e.target.checked })} className="h-4 w-4" /><span className="text-slate-700">{t("Ενεργό", "Active")}</span></label>
              {err && <div className="rounded-lg bg-rose-50 px-3 py-2 text-xs text-rose-700">{err}</div>}
              <div className="flex justify-end gap-2"><button onClick={() => setEc(null)} className="px-3 py-2 text-sm text-slate-400">{t("Άκυρο", "Cancel")}</button>
                <button onClick={() => saveC.mutate(ec)} disabled={ec.code.length < 3 || saveC.isPending} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">{t("Αποθήκευση", "Save")}</button></div>
            </div>
          </div>
        </div>
      )}
      ...
      {eb && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={() => setEb(null)}>
          <div className="max-h-[85vh] w-full max-w-md overflow-auto rounded-2xl bg-white p-5" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between"><div className="font-semibold text-slate-800">{eb._id ? t("Επεξεργασία πακέτου", "Edit bundle") : t("Νέο πακέτο", "New bundle")}</div><button onClick={() => setEb(null)} className="text-slate-400"><X className="h-4 w-4" /></button></div>
            <div className="space-y-3">
              <label className="block text-sm"><span className="mb-1 block text-slate-600">{t("Όνομα", "Name")}</span>
                <input value={eb.name} onChange={(e) => setEb({ ...eb, name: e.target.value })} placeholder={t("π.χ. 2+1 σε αντηλιακά", "e.g. 2+1 on sunscreens")} className="w-full rounded-lg border border-slate-300 px-3 py-2" /></label>
              <div className="flex gap-2">
                {([["nplusm", t("«2+1» ίδιο είδος", "«2+1» same item")], ["combo", t("Combo (μαζί)", "Combo (together)")]] as const).map(([k, l]) => (
              ...
              {eb.kind === "nplusm" ? (
                <>
                  <label className="block text-sm"><span className="mb-1 block text-slate-600">{t("Barcode προϊόντος", "Product barcode")}</span>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <label className="block text-sm"><span className="mb-1 block text-slate-600">{t("Αγοράζει", "Buy")}</span>
                    ...
                    <label className="block text-sm"><span className="mb-1 block text-slate-600">{t("Δωρεάν", "Free")}</span>
                  ...
                  <p className="rounded-lg bg-emerald-50 px-3 py-2 text-xs text-emerald-800">{t(`Ανά ${(eb.buy_qty ?? 2) + (eb.free_qty ?? 1)} τεμάχια στο καλάθι, τα ${eb.free_qty} είναι δωρεάν.`, `For every ${(eb.buy_qty ?? 2) + (eb.free_qty ?? 1)} units in the cart, ${eb.free_qty} are free.`)}</p>
                </>
              ) : (
                <>
                  <div className="text-sm"><span className="mb-1 block text-slate-600">{t("Barcodes που πρέπει να είναι ΟΛΑ στο καλάθι", "Barcodes that must ALL be in the cart")}</span>
                    ...
                    <button onClick={() => setEb({ ...eb, lines: [...(eb.lines ?? []), { barcode: "", qty: 1 }] })} className="mt-1.5 inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2 py-1 text-xs text-slate-600"><Plus className="h-3 w-3" /> {t("Γραμμή", "Line")}</button>
                  </div>
                  <label className="block text-sm"><span className="mb-1 block text-slate-600">{t("Έκπτωση στα είδη του πακέτου:", "Discount on the bundle items:")} <b>{eb.discount_pct}%</b></span>
              ...
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={eb.active} onChange={(e) => setEb({ ...eb, active: e.target.checked })} className="h-4 w-4" /><span className="text-slate-700">{t("Ενεργό", "Active")}</span></label>
              {err && <div className="rounded-lg bg-rose-50 px-3 py-2 text-xs text-rose-700">{err}</div>}
              <div className="flex justify-end gap-2"><button onClick={() => setEb(null)} className="px-3 py-2 text-sm text-slate-400">{t("Άκυρο", "Cancel")}</button>
                <button onClick={() => saveB.mutate(eb)} disabled={!eb.name.trim() || saveB.isPending} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">{t("Αποθήκευση", "Save")}</button></div>
            </div>
          </div>
        </div>
      )}
