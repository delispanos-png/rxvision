...
## 2026-09-24 — RxVision Connect

**D-035 · Το «ανοιχτό υπόλοιπο» είναι ΠΑΡΑΓΩΓΟ, όχι αποθηκευμένος μετρητής.**
`ConnectRepository.balances()` το υπολογίζει κάθε φορά από κινήσεις − επιστροφές − τακτοποιήσεις.
*Why:* ένας μετρητής που ξεσυγχρονίζεται παράγει **διαφωνία μεταξύ δύο φαρμακείων** — το
χειρότερο δυνατό σφάλμα σε κύκλωμα εμπιστοσύνης. Ίδια αρχή με τον watermark του συγχρονισμού:
η αλήθεια βγαίνει από τα δεδομένα, όχι από μετρητή.

**D-034 · Η κράτηση γεννιέται στην ΑΠΟΔΟΧΗ και λήγει μόνη της· το απόθεμα αλλάζει στην ΠΑΡΑΔΟΣΗ.**
Τρεις διακριτές στιγμές: εμφάνιση (τίποτα) → αποδοχή (κράτηση, beat κάθε 5΄) → «Παρέδωσα»
(απόθεμα + ledger και στα δύο φαρμακεία).
*Why:* αν δέσμευε η εμφάνιση, ένα αίτημα που δεν προχώρησε θα κρατούσε κλειδωμένο ξένο ράφι. Αν
το απόθεμα άλλαζε στην αποδοχή, μια συμφωνία που δεν υλοποιήθηκε θα άφηνε δύο λάθος αποθέματα.

**D-033 · Άγνωστο απόθεμα → `None`, ΟΧΙ μηδέν — και τότε απαντά άνθρωπος.**
`connect_availability.available_qty()` επιστρέφει `None` όταν το είδος δεν παρακολουθείται.
*Why:* **13 από τα 41.127 είδη** έχουν καταχωρημένο απόθεμα (μέτρηση 24/09/2026). Αν το άγνωστο
γινόταν μηδέν, το κύκλωμα θα έλεγε «κανείς δεν μπορεί να καλύψει» σε 15/15 φαρμακεία από την
πρώτη μέρα και θα έμοιαζε χαλασμένο. Απόφαση ιδιοκτήτη: ό,τι ξέρουμε απαντά αυτόματα, τα
υπόλοιπα ρωτούν τον φαρμακοποιό.

**D-032 · Οι κρατήσεις είναι ΚΑΘΟΛΙΚΕΣ, τα ποσοστά ανά δίκτυο.**
Το καθολικό όριο υπερισχύει πάντα του ποσοστού ομάδας· οι ενεργές κρατήσεις αφαιρούνται
ανεξαρτήτως δικτύου.
*Why:* αλλιώς δύο δίκτυα θα υπόσχονταν το ίδιο τεμάχιο (σενάριο 13 της προδιαγραφής) και το
φαρμακείο θα δεσμευόταν δύο φορές για κάτι που έχει μία.

**D-031 · Το Connect έχει ΔΙΚΕΣ ΤΟΥ ομάδες, αλλά ΚΟΙΝΟ μηχανισμό εισόδου.**
Απόφαση ιδιοκτήτη: ξεχωριστές ομάδες από τη συνομιλία. Για να μην υπάρξουν δύο αποκλίνοντες
μηχανισμοί, η εύρεση με ΑΦΜ και ο φρουρός συμμετοχής βγήκαν σε `services/pharmacy_directory.py`
και τους χρησιμοποιούν **και τα δύο** κυκλώματα.
*Why:* δύο αντίγραφα του «βρες φαρμακείο με ΑΦΜ» σημαίνουν ότι κάποιο φαρμακείο θα βρίσκεται
στο ένα κύκλωμα και «δεν θα υπάρχει» στο άλλο, χωρίς εξήγηση.

**D-030 · Το Connect χρησιμοποιεί `portal:manage`, ΟΧΙ νέο δικαίωμα `connect:*`.**
*Why:* τα δικαιώματα των ρόλων ζουν στη ΒΑΣΗ κάθε φαρμακείου και δεν ενημερώνονται αναδρομικά.
Νέο κλειδί θα άφηνε όλους πλην του ιδιοκτήτη έξω από πληρωμένο κύκλωμα — ακριβώς το λάθος που
μπλόκαρε 14 στα 15 φαρμακεία στη δοκιμή δυνατότητας.

---

## 2026-09-23 — πρόσθετα, συνομιλία φαρμακείων, σημειώσεις έκδοσης

**D-029 · Το `catalog_seed` εξαιρείται της 14ήμερης δοκιμής — αντ' αυτού «δείγμα».**
`addon_service.NO_TRIAL = {"catalog_seed"}`· ο πελάτης φορτώνει 10 προϊόντα ανά κατηγορία και
τα αφαιρεί καθαρά (μόνο όσα δεν πείραξε).
*Why:* η δοκιμή σε κύκλωμα που **παραδίδει δεδομένα** δεν αντιστρέφεται — μετά τις 14 ημέρες ο
πελάτης κρατά ολόκληρο τον κατάλογο. Το δείγμα δείχνει ποιότητα χωρίς να παραδίδει την αξία.

**D-028 · Η συνομιλία φαρμακείων ζει στις Ρυθμίσεις· καθημερινό σημείο εισόδου = το Topbar.**
Το κύκλωμα βγήκε από το κύριο μενού· τα αδιάβαστα φαίνονται σε εικονίδιο δίπλα στις ειδοποιήσεις.
*Why:* οι ομάδες και οι προσκλήσεις ρυθμίζονται μία φορά — δεν δικαιολογούν θέση στο μενού που
κοιτάς κάθε μέρα. Τα μηνύματα, όμως, θέλουν ορατότητα παντού, όχι μόνο σε μία σελίδα.

**D-027 · Η συνομιλία είναι δωρεάν core module, αλλά η πρόσκληση απαιτεί ΕΝΕΡΓΗ συνδρομή.**
Το module μπαίνει στα `_CORE_MODULES`· το `_subscription_block()` απορρίπτει πρόσκληση προς ΑΦΜ
χωρίς ενεργή συνδρομή — **η δοκιμαστική δεν αρκεί**, εκτός αν παραχωρηθεί ρητά.
*Why:* η αξία του δικτύου είναι ο λόγος να παραμείνεις συνδρομητής. Αν η δοκιμή έδινε πλήρη
πρόσβαση στο δίκτυο, το δίκτυο θα γινόταν ο φτηνός τρόπος να το χρησιμοποιείς χωρίς να πληρώνεις.

**D-026 · Η σειρά των εκδόσεων αποθηκεύεται ως συμπληρωμένη συμβολοσειρά (`vsort`), όχι πίνακας.**
`_vsort("1.48.2") → "001.048.002"`.
*Why:* η **MongoDB δεν συγκρίνει πίνακες στοιχείο-προς-στοιχείο** όπως η Python — χρησιμοποιεί το
μέγιστο στοιχείο. Έτσι η 1.37.189 έβγαινε πριν από την 1.48.2 και το χρονολόγιο ήταν ανάκατο.

**D-025 · «Ενεργό χωρίς πληρωμή» = απουσία γραμμής τιμολογίου `addon:<id>` — ΟΧΙ απουσία κάρτας.**
`addon_payment_amnesty.py`.
*Why:* το πρώτο κριτήριο («δεν έχει κάρτα») θα ανακαλούσε πρόσθετο πελάτη που **είχε πληρώσει**
με άλλο τρόπο (ΜΠΙΝΙΚΟΣ: `viva_transaction_id` + εξοφλημένο τιμολόγιο 970 €). Η πληρωμή
αποδεικνύεται από το παραστατικό, όχι από τον τρόπο πληρωμής.

**D-024 · Η ενεργοποίηση χρεώσιμου προσθέτου χρεώνει ΑΜΕΣΩΣ, αναλογικά — αλλιώς δεν ανοίγει.**
`billable_gate.py` (ένα σημείο απόφασης για `addons`/`eshop`/`messaging_topup`) +
`_prorated_addon()`/`activation_quote()` στο `addon_service`.
*Why:* η προηγούμενη συμπεριφορά έβαζε το κόστος μόνο στην **ανανέωση**, οπότε ετήσιος πελάτης
που άνοιγε πρόσθετο στη μέση της περιόδου το είχε δωρεάν ως τη λήξη (μετρημένη τρύπα ~86,58 €).
Τα seats και η αλλαγή πλάνου χρέωναν ήδη αναλογικά — τα πρόσθετα ήταν η εξαίρεση, όχι ο κανόνας.
...
**D-0xx · Η ποσότητα που δόθηκε είναι δεδομένο, όχι σημαία (v1.55.0).**
Νέο `prescription_items.executed_qty` (πάντα ακέραιο). Το `is_executed` σημαίνει πλέον ρητά
«πλήρως εκτελεσμένη». *Why:* η ΗΔΥΚΑ δίνει «Υπόλοιπο» ανά γραμμή — μια συσκευασία 2 τεμαχίων με
το 1 δοσμένο ΔΕΝ είναι ανεκτέλεστη. Η σύμπτυξη σε ναι/όχι χρέωνε ολόκληρη τη γραμμή ως χαμένη
(232.885 € ψεύτικης απώλειας σε 17.093 γραμμές) και έκρυβε το φάρμακο από τους ελέγχους
αλληλεπίδρασης. Απορρίφθηκε η επανα-άντληση: το `details.outstanding` υπήρχε ήδη στη βάση.

**D-0xx · «Ανακτήσιμο» = ΛΟΓΟΣ + ΡΟΛΟΪ, σε ένα σημείο (v1.56.0).**
`services/recoverable.py` (`mongo_filter`/`reason`/`RECOVERABLE_CASE`), κατώφλι = ΑΡΧΗ ΗΜΕΡΑΣ.
*Why:* η περίπτωση εκτέλεσης μόνη της δεν αρκεί — 25.217 από 26.778 «ανακτήσιμες» είχαν ήδη
λήξει. Ρητά ΔΕΝ εφαρμόζεται σε ιστορικές μετρήσεις: «χάθηκε» (γεγονός) ≠ «ανακτήσιμο» (πράξη
σήμερα).

**D-0xx · Τα μηνύματα υπακούν στη νομοθεσία, όχι στην κοινή λογική (v1.57.x).**
Το υπόλοιπο ΜΙΑΣ εκτέλεσης κλειδώνει στο φαρμακείο που την ξεκίνησε· οι επόμενες εκτελέσεις
είναι ελεύθερες. *Why:* ο Σύμβουλος έλεγε ότι τα ανεκτέλεστα «θα καταλήξουν σε άλλο φαρμακείο»
— ψευδές. Ο κανόνας μπήκε και στο SYSTEM prompt του Copilot, γιατί χωρίς αυτόν το μοντέλο
παράγει μόνο του τη λάθος (κοινώς λογική) παραδοχή.

**D-0xx · Το μέλος ομάδας είναι ΨΕΥΔΩΝΥΜΟ, όχι εγγραφή ασθενή (v1.58.0).**
`patient_groups.members[].pseudo_id` = HMAC(ΑΜΚΑ, pepper). *Why:* είναι ντετερμινιστικό, οπότε
δέχεται ΑΜΚΑ ατόμου χωρίς δεδομένα και το μέλος συνδέεται μόνο του με την πρώτη συνταγή — το
κλειδί για δομή που μπαίνει με 40 τροφίμους. Το ακατέργαστο ΑΜΚΑ δεν αποθηκεύεται ποτέ.
⚠ `patients_anonymized._id` (ObjectId) ≠ `pseudo_id`: οι εκτελέσεις δείχνουν στο πρώτο.

**D-0xx · Οι χρεώσεις δομής ΠΑΡΑΓΟΝΤΑΙ, δεν αποθηκεύονται (v1.59.0).**
Χρέωση = Σ `patient_share` των εκτελέσεων των μελών, τη στιγμή που ρωτάμε. Αποθηκεύονται μόνο
εισπράξεις και χειροκίνητες χρεώσεις. *Why:* γραμμές ανά εκτέλεση θα ήθελαν συγχρονισμό, και
κάθε επανα-άντληση θα άφηνε διπλή ή ξεπερασμένη γραμμή. Ρητά ΚΑΜΙΑ κατανομή πληρωμής σε
συνταγές (ούτε FIFO) — δεν ξέρουμε τι πλήρωσε τι, και δεν το κατασκευάζουμε.

**D-0xx · Η δομή χρωστά ΜΟΝΟ τη συμμετοχή (v1.59.0).**
Ό,τι καλύπτει το ταμείο τρέχει αμετάβλητο στην Αποζημίωση και δεν εμφανίζεται στην οθόνη της
δομής. *Why:* διευκρίνιση ιδιοκτήτη — η δομή είναι πελάτης σαν κάθε άλλον· η μόνη διαφορά είναι
ότι δεν κόβεται απόδειξη εκείνη τη στιγμή.

**D-0xx · Η πύλη ακολουθεί ΣΥΓΚΑΤΑΘΕΣΗ, όχι συγγένεια (v1.60.0).**
`services/portal_access.py`: γονική μέριμνα (αυτόματη, παύει στα 18 μόνη της) · εξουσιοδότηση
(ρητή, ανακλητή) · ο εαυτός. Ενήλικο μέλος ΔΕΝ βλέπει άλλο ενήλικο επειδή «είναι οικογένεια».
*Why:* θα ήταν γνωστοποίηση δεδομένων υγείας. Η ηλικία υπολογίζεται σε κάθε ανάγνωση από το
ΑΜΚΑ (`utils/amka.py`)· αστοχία → ενήλικος (fail closed).

**D-0xx · ΜΙΑ μηχανή λιστών για δύο κυκλώματα (v1.62.0).**
`services/group_lists.py`. *Why:* τα ίδια ερωτήματα απαντούν στο «τι πήρε ο μπαμπάς» και στο
«τι να έχω έτοιμο για 40 τροφίμους». Δύο αντίγραφα θα απέκλιναν — και οι κανόνες δεν είναι
προφανείς (ανακτήσιμα ≠ ανεκτέλεστα, υπόλοιπο τεμαχίων ≠ ποσότητα, next_open_date ≠ αφαίρεση
θέσεων αλυσίδας).
