...
PWA · Docker/Caddy/Vault) for pharmacy prescription analytics (ΗΔΙΚΑ / ΓΕΣΥ), handling
...
that: analytics are **numerically wrong for real ΗΔΙΚΑ tenants** (wholesale price hardcoded
...
| 2 | Fix wholesale pricing → correct profitability (T-06) | Core selling feature gives **wrong numbers** on real ΗΔΙΚΑ data | Critical | M |
...
| 11 | Verify ΗΔΙΚΑ repeat mapping | Wrong future-prescription forecasts → bad order suggestions | High | M |
...
| 16 | SSRF allow-list on ΗΔΙΚΑ `base_url` (M2) | Tenant could probe internal network / exfil platform key | Medium | S |
...
2. **Then data correctness:** fix wholesale pricing (T-06) and verify the ΗΔΙΚΑ repeat
