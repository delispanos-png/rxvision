...
> **25/09/2026 — v1.62.0 (deployed, c060100).** Νέο κύκλωμα ομάδων ασφαλισμένων:
> **Οικογένειες** (πρόσθετο 15 €/μ, λίστα με αναζήτηση & ανάλυση ανά μέλος) ·
> **Δομές Φροντίδας** (20 €/μ, δικό τους σημείο στην ΑΝΑΛΥΣΗ — `/care`: πέντε φύλλα εργασίας,
> λογαριασμός με μεταφορά υπολοίπου, λίστα & οδηγίες λήψης με εκτύπωση/email, στοιχεία δομής) ·
> **Πρόσβαση Πύλης** (γονική μέριμνα που παύει στα 18 μόνη της + εξουσιοδοτήσεις φροντίδας).
> Μηχανή λιστών κοινή: `services/group_lists.py`. Σχεδιασμός: `docs/patient-groups-design.md`.
> ΕΚΚΡΕΜΕΙ από 24/09: φύλακας πληρότητας εκτελέσεων (363 συνταγές / 521 εκτελέσεις λείπουν).

> **24/09/2026 — v1.57.1 (deployed).** Μεγάλος κύκλος «λέει την αλήθεια το RxVision;»:
> (α) `prescription_items.executed_qty` — η μερική εκτέλεση δεν είναι πια ανεκτέλεστη
> (backfill 1.055.828 γραμμών· −232.885 € ψεύτικης απώλειας)· (β) `services/recoverable.py` —
> «ανοιχτή» ≠ «ανακτήσιμη» (26.778 → 1.561)· (γ) νομική διόρθωση μηνυμάτων: το υπόλοιπο ΜΙΑΣ
> εκτέλεσης κλειδώνει στο φαρμακείο, οι επόμενες είναι ελεύθερες.
> Έλεγχος ΗΔΥΚΑ σε 25 ζωντανές συνταγές: **11/14 πεδία ταυτίζονται, μηδέν πραγματικά λάθη**.
> ΕΚΚΡΕΜΕΙ: φύλακας πληρότητας (363 συνταγές / 521 εκτελέσεις λείπουν). Απόθεμα: παγωμένο.

> Living snapshot for session continuity. Update at the end of each work session.
> Last updated: **2026-09-24**.

## 🗓️ 2026-09-24 — RxVision Connect (v1.51.0)

**Κλάδος:** `feature/loyalty-portal-build`. Ημερολόγιο: `docs/ai/journal/2026-09-24.md`.
Σχέδιο & αναφορά: `docs/connect/00-audit-and-design.md`, `docs/connect/01-reference.md`.

**Νέο πληρωμένο κύκλωμα (30 €/μ · 300 €/έτος):** δίκτυα συνεργασίας φαρμακείων. Δικά σου δίκτυα
με πρόσκληση **στο ΑΦΜ** και αποδοχή· αίτημα για σκεύασμα σε ένα ή περισσότερα δίκτυα· **μερική
κάλυψη** από πολλούς· κράτηση με λήξη· κίνηση που ενημερώνει το **υπάρχον** απόθεμα και το
ledger **και των δύο** φαρμακείων· ανοιχτά υπόλοιπα, επιστροφή σε είδος, τακτοποίηση αξίας,
διαφωνία.

**Το εύρημα που καθόρισε τον σχεδιασμό:** μόνο **13 από τα 41.127 είδη** έχουν καταχωρημένο
απόθεμα. Άρα η διαθεσιμότητα είναι **υβριδική**: ό,τι ξέρουμε απαντά αυτόματα (μέσα σε ποσοστό
ανά δίκτυο ∧ καθολικό όριο ∧ απόθεμα ασφαλείας ∧ κρατήσεις), τα υπόλοιπα ρωτούν τον άνθρωπο.
Άγνωστο απόθεμα = `None`, **ποτέ μηδέν**.

**Τεκμηριωμένη εξαίρεση:** διατενάντ συλλογές εκτός `BaseRepository` (ίδιο μοτίβο με το
`pharmacy_chat`), με ΜΙΑ δήλωση ορατότητας και τεστ που τη φυλάει.

**Επαλήθευση:** 34 έλεγχοι — όλα τα 20 σενάρια της προδιαγραφής — σε πραγματική βάση.

---

## 🗓️ 2026-09-23 — τρία νέα κυκλώματα + κλειστός κύκλος χρέωσης (v1.41 → v1.50.0)

**Κλάδος:** `feature/loyalty-portal-build`. Ημερολόγιο: `docs/ai/journal/2026-09-23.md`.

**Νέα κυκλώματα:** «Προχορηγήσεις (δανεικά)» 25 €/μ — πελάτης από λίστα, σάρωση **χωρίς κανένα
πάτημα** (QR & ταινία ΕΟΦ), ολόκληρο το GS1, «πότε θα φέρει τη συνταγή» + λίστα ημέρας + σήμα
Συμβούλου· «Έτοιμος κατάλογος ειδών» 15 €/μ — επιλογή προϊόντων ανά φαρμακείο, καθημερινός
συγχρονισμός που **δεν αγγίζει ποτέ** αποθέματα/εκπτώσεις/χειροκίνητες τιμές, αναφορά αλλαγών,
δείγμα αντί δοκιμής· «Συνεργαζόμενα φαρμακεία» — **δωρεάν σε όλους**, ομάδες, πρόσκληση με ΑΦΜ,
μήνυμα σε όλους ή σε έναν, εικονίδιο στο Topbar, ζει στις **Ρυθμίσεις**.

**Χρέωση προσθέτων — έκλεισε.** `billable_gate` (ένας φρουρός κάρτας) + **αναλογική χρέωση στην
ενεργοποίηση** (αποτυχία χρέωσης = δεν ανοίγει) + ανάκληση όσων ήταν ενεργά χωρίς πληρωμή. Το
κριτήριο ΔΕΝ είναι «δεν έχει κάρτα» αλλά **απουσία γραμμής τιμολογίου `addon:<id>`**.

**Επίσης:** «σε ποιον δώσαμε αυτό το κουτί;» (σάρωση → ασθενής/συνταγή· barcode → όλοι όσοι το
πήραν, για ανάκληση παρτίδας)· «Τι νέο υπάρχει» με **122 εκδόσεις** και «πού θα το βρεις»· ο
Σύμβουλος ως λίστα που αδειάζει· AI κατηγοριοποίηση παραφαρμάκων **28.296/28.349 (99,8%)**·
κρυπτογράφηση 18 μυστικών με ανεξάρτητο «Κλειδί Μυστικών».

**Συμβάν:** build ενώ έτρεχε deploy → μοιρασμένος στόλος, άστυλη εφαρμογή. Νέος μόνιμος κανόνας.

---

## 🗓️ 2026-09-21 — ορατότητα add-ons & δοκιμές δυνατοτήτων (v1.40.0)

**Κλάδος:** `feature/loyalty-portal-build`. Ημερολόγιο: `docs/ai/journal/2026-09-21.md`.

**Θέμα της ημέρας: η βάση είναι η αλήθεια, όχι ο κώδικας.** Ο Σύμβουλος («Ο Σύμβουλός σου») είχε
γίνει `active: True` στον κώδικα και είχε γίνει deploy, αλλά κανένας πελάτης δεν τον έβλεπε: ο
κατάλογος add-ons ζει στη **βάση** και το seed είναι `$setOnInsert` (σκόπιμα — προστατεύει τιμές
που άλλαξε ο admin). Δεύτερο, ανεξάρτητο φίλτρο: `packages.available_addons`. Και τα δύο
διορθώθηκαν σε επίπεδο δεδομένων — χωρίς deploy, δεν υπάρχει cache.

**Χρέωση ≠ πρόσβαση.** Ο ΜΠΙΝΙΚΟΣ χρεωνόταν 100 €/έτος για κύκλωμα κλειδωμένο στα
`tenants.modules`. Δύο χειριστήρια του adminpanel γράφουν διαφορετικά πράγματα και δεν
συνομιλούν· ο `resolve_modules` δεν κοιτάζει ποτέ το `subscriptions.addons`. Ξεκλειδώθηκε.
Έλεγχος σε όλο το πελατολόγιο: 1 «χρεώνεται χωρίς πρόσβαση», 2 «πρόσβαση χωρίς χρέωση».

**Νέο κύκλωμα — `/admin/module-trials`.** Ποιος δοκιμάζει τι, πότε το πήρε, πότε του λήγει, αν το
αγόρασε· + αυτόματη ενημέρωση αγοράς (beat 09:00 UTC, default 3 και 1 ημέρα πριν τη λήξη, dedup
ανά κατώφλι). Το `start_trial()` καταγράφει πλέον και την ΕΝΑΡΞΗ. Παγίδα που λύθηκε: η λήξη
είναι «τεμπέλικη» (μόνο στο login), οπότε η κατάσταση βγαίνει από την ημερομηνία.

**Ανοιχτό (απόφαση ιδιοκτήτη):** αναλογική χρέωση στο `activate()`· φύλακας στον πίνακα Modules·
καθαρισμός δωρεάν παραχωρήσεων. Λεπτομέρειες στο `docs/todo.md`.

---

## 🗓️ 2026-09-10 — κατάσταση κλεισίματος ημέρας

**Κλάδος:** `feature/loyalty-portal-build` · 12 commits (`be35f52` → `d4f0ed3`) · deployed σε
10.0.0.5/6/7. Ημερολόγιο: `docs/ai/journal/2026-09-10.md`.

**Θέμα της ημέρας: σιωπηλές αποτυχίες.** Τρία ανεξάρτητα προβλήματα ήταν το ίδιο μοτίβο —
αποτυχία που επιστρέφει «ok», έλεγχος σε μία μόνο διαδρομή, σημαία που μπήκε μετά τα δεδομένα της.

- **Εμβολιασμοί**: η ΗΔΥΚΑ θέλει `GET /users/me` (server-side session) πριν από κάθε κλήση μητρώου·
  8/9 φαρμακεία είχαν ΜΗΔΕΝ. Backfill +4.431 εγγραφές. Σύγχρονισμός τώρα κάθε 6 ώρες, με ίχνος
  στο `sync_jobs` και σφάλματα που ΡΙΧΝΟΥΝ αντί να σιωπούν.
- **Ειδοποιήσεις ασθενή**: νέο `notify_prefs` (SMS/Viber/email/push) με τον έλεγχο ΜΕΣΑ στους
  αποστολείς· μπλοκαρισμένο μήνυμα = μηδέν χρέωση.
- **AI credits**: ενοποίηση σε ευρώ· η αγορά ήταν σπασμένη (πεδίο `questions` vs `credit_cents`).
- **Onboarding**: `POST /ingestion/hdika/setup` — μόνο κωδικοί ΗΔΥΚΑ, όλα τα υπόλοιπα αυτόματα,
  ιστορικό 2 έτη + τρέχον.
- **Παγωμένος συγχρονισμός**: 1 email/ημέρα έως 5 ημέρες + μηνιαία πρόληψη· KPI στο adminpanel.

**Υγεία υποδομής:** 9/11 φαρμακεία συγχρονίζονται εντός 20λέπτου. 2 παγωμένα (CLINICAL 350h
κλείδωμα, ΚΑΙΣΑΣ 245h/401) — θέλουν τηλέφωνο. Demo tenant έτοιμο για την παρουσίαση 11/09.
